...
\renewcommand{\thesection}{\Alph{section}} 
    ...
    %       - an:cmake_super
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{CMake principal}\label{an:cmake_super}
\paragraph{}
...
\newpage
\section{Valeurs constantes des MDCT}\label{an:mdct_const}
\paragraph{}
...
\newpage
\section{Algorithmes de référence}\label{an:mdct_ref}
\subsection{MDCT de référence en \emph{float}}\label{an:mdct_ref_float}
\paragraph{}
...
\subsection{MDCT de référence en \emph{integer}}\label{an:mdct_ref_int}
\paragraph{}
...
\newpage
\section{Génération d'un signal sinusoïdal}\label{an:sin_wave}
\subsection{Génération d'un signal sinusoïdal en \emph{float}}\label{an:sin_wave_float}
\paragraph{}
...
\subsection{Génération d'un signal sinusoïdal en \emph{integer}}\label{an:run_ref_mdct_fint}
\paragraph{}
...
\newpage
\section{Implémentation de la MDCT basée sur la FFT de \emph{FFTW3}}\label{an:mdct_fftw3_f32}
\subsection{Header}\label{an:mdct_fftw3_f32_header}
\paragraph{}
...
\subsection{Constructeur}\label{an:mdct_fftw3_f32_constructor}
\paragraph{}
...
\subsection{Destructeur}\label{an:mdct_fftw3_f32_destructor}
\paragraph{}
...
\subsection{Fonction MDCT}\label{an:mdct_fftw3_f32_func}
\paragraph{}
...
\newpage
\section{Validation de la MDCT \emph{FFTW3} en \emph{float 32}}\label{an:fftw3_example}
\subsection{Code source}\label{an:fftw3_example_code}
\paragraph{}
...
\subsection{Compilation}\label{an:fftw3_example_compilation}
\paragraph{}
...
\newpage
\section{Implémentation de la MDCT basée sur la FFT de \emph{Ne10} en \emph{floating point}}\label{an:mdct_ne10_f32}
\subsection{Header}\label{an:mdct_ne10_f32_header}
\paragraph{}
...
\subsection{Constructeur}\label{an:mdct_ne10_f32_constructor}
\paragraph{}
...
\subsection{Destructeur}\label{an:mdct_ne10_f32_destructor}
\paragraph{}
...
\subsection{Fonction MDCT}\label{an:mdct_ne10_f32_func}
\paragraph{}
...
\end{lstlisting}
    ...
    %   8.4 Performances
    %       - an:perf_fft_ne10
    %           - an:perf_fft_ne10_code
    %           - an:perf_fft_ne10_cmake
    %       - an:perf_fft_fftw3
    %           - an:perf_fft_fftw3_code
    %           - an:perf_fft_fftw3_cmake
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\newpage
\section{Mesure des performances des FFT de \emph{Ne10}}\label{an:perf_fft_ne10}
\subsection{Code source}\label{an:perf_fft_ne10_code}
\paragraph{}
Code permettant de tester la vitesse d'exécution moyenne de différentes FFT proposées par la librairie \emph{Ne10}. La moyenne est calculée sur 10.000.000 exécutions avec des données aléatoires en entrée de la FFT différentes pour chaque exécution. Les variables de préprocesseur définies à la compilation permettent sur base du même code de mesurer le temps d'exécution moyen avec écart type :
\begin{itemize}
...
\subsection{Compilation}\label{an:perf_fft_ne10_cmake}
\paragraph{}
...
\newpage
\section{Mesure des performances des FFT de \emph{FFTW3}}\label{an:perf_fft_fftw3}
\subsection{Code source}\label{an:perf_fft_fftw3_code}
\paragraph{}
Code permettant de tester la vitesse d'exécution moyenne de la FFT en \emph{float 32} de la librairie \emph{FFTW3}. La moyenne est calculée sur 10.000.000 exécutions avec des données aléatoires en entrée de la FFT différentes pour chaque exécution.
\lstset{language=C++}
\begin{lstlisting}
#include <iomanip>
#include <iostream>
...
\subsection{Compilation}\label{an:perf_fft_fftw3_cmake}
\paragraph{}
...
\newpage
\section{Implémentation de la MDCT basée sur la FFT de \emph{Ne10} en \emph{fixed point}}\label{an:mdct_ne10_i32_c}
\subsection{Header}\label{an:mdct_ne10_i32_c_header}
\paragraph{}
...
\subsection{Constructeur}\label{an:mdct_ne10_i32_c_constructor}
\paragraph{}
...
\subsection{Destructeur}\label{an:mdct_ne10_i32_c_destructor}
\paragraph{}
...
\subsection{Fonction MDCT}\label{an:mdct_ne10_i32_c_func}
\paragraph{}
...
\end{lstlisting}
    ...
    %       - an:mdct_ne10_i32_neon
    %       - an:mdct_ne10_i32_neon_header
    %       - an:mdct_ne10_i32_neon_constructor
    %       - an:mdct_ne10_i32_neon_destructor
    %       - an:mdct_ne10_i32_neon_func
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\newpage
\section{Implémentation de la MDCT basée sur la FFT de \emph{Ne10} en \emph{fixed point} avec optimisations \emph{Neon}}\label{an:mdct_ne10_i32_neon}
\subsection{Header}\label{an:mdct_ne10_i32_neon_header}
\paragraph{}
Header de la classe \texttt{mdct\_ne10\_i32\_neaon} : MDCT basée sur la FFT de la librairie \emph{Ne10} en \emph{integer} (32 bits) optimisée par l'utilisation des opérations SIMD Neon. La classe contient les structures de données \texttt{fft\_in} en représentation Q1.15 et \texttt{fft\_out} en Q9.15, les tableaux de facteurs de \texttt{twiddle} utilisé pour le pre- et le post-processing et la configuration de la FFT (\texttt{cfg}). Contrairement aux autres implémentations, les facteurs de twiddle non sont pas rassemblés dans un seul tableau. Les tableaux de facteurs de \emph{pre-twiddling} et de \emph{post-twiddling} sont séparés car ils sont utilisés en 16 bits pour le \emph{pre-twiddling} et en 32 bits pour le \emph{post-twiddling}. Chacun de ses tableaux est séparé en deux car pour que chaque moitié puisse être initialisée dans un ordre qui facilite l'utilisation des opérations SIMD. L'implémentation des fonctions de ce header est présentée dans les annexes suivantes.
\lstset{language=C++}
\begin{lstlisting}
#pragma once
...
\subsection{Constructeur}\label{an:mdct_ne10_i32_neon_constructor}
\paragraph{}
...
\subsection{Destructeur}\label{an:mdct_ne10_i32_neon_destructor}
\paragraph{}
Destructeur de la classe \texttt{mdct\_ne10\_i32\_c} qui permet de libérer la mémoire allouée à la configuration de la FFT avec la fonction appropriée de la librairie \emph{Ne10}.
\lstset{language=C++}
\begin{lstlisting}
...
\subsection{Fonction MDCT}\label{an:mdct_ne10_i32_neon_func}
\paragraph{}
