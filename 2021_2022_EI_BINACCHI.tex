...
%    \url{https://sci-hub.se/https://www.jstor.org/stable/29775194#}
    ...
    \begin{itemize}
        \item La configuration et les tableaux d'entrée et de sortie de la FFT sont définies avec les types \emph{int32} et plus \emph{float32} de la librairie \emph{Ne10};
    ...
    \subsubsection{Utilisation de la FFT \emph{Ne10} en \emph{float 32}}
    \paragraph{}
    ...
    \begin{itemize}
        ...
        \item La fonction de FFT est appelée avec en paramètres le tableau contenant les données d'entrée, le tableau dans lequel sera calculé le résultat de la FFT, la configuration préalablement initialisée, un \emph{integer} qui permet de réaliser la FFT ou son opération inverse et, en plus de la FFT en \emph{float}, un facteur de mise à échelle mis ici à $0$.
    \end{itemize}

    \paragraph{}
    Il est à noter que la valeur à attribuer au facteur de mise à échelle de la fonction de FFT n'est pas documenté dans la librairie \emph{Ne10}. Je l'ai mis à 0 en testant la FFT de \emph{Ne10} dans le but d'obtenir les mêmes valeurs qu'en \emph{float} mises à une échelle Q15.
    ...
    \paragraph{}
    L'initialisation de la MDCT se fait dans le constructeur de la classe \texttt{mdct\_ne10\_i32\_c} présentée à l'\textbf{annexe J.2} :
    \begin{itemize}
        \item Le tableau de facteurs de \texttt{twiddle} est initialisé en \emph{double} puis converti en \emph{integer} (représentation Q15) : l'initialisation n'est pas faite en \emph{fixed point} car le \emph{double} permet de conserver une plus grande précision et que l'initialisation n'a pas besoin d'être optimisée;
        \item La FFT est initialisée de la même manière que la FFT en \emph{floating point} : elle est initialisée a
    \end{itemize}
