...
\begin{document}
    ...
    \paragraph{}
    Tout au long de ce travail, les MDCT ont été validées avec divers signaux en entrée : des signaux à une ou plusieurs fréquences connues ou des données aléatoires. La section \ref{sec:validation} consacrée au protocole de validation donne plus de détails sur la génération des spectres de fréquences produits par les MDCT et leur interprétation. Pour faciliter la lecture des résultats et les comparaison ce travail présentera principalement des tests effectués avec en entrée un signal à \SI{440}{\hertz}.
    ...
    \paragraph{}
    La figure \ref{fig:signal_f32_f64} offre une représentation graphique des signaux sinusoïdaux en \emph{floating point}. Le signal généré en \emph{float} (en pointillés) se superpose parfaitement au signal généré en \emph{double}. La période du signal est de \SI{5}{\milli\second} correspondant à la fréquence de \SI{200}{\hertz} attendue.



    \paragraph{}
    Les valeurs maximale et minimale du signal sont de $0.9$ et $-0.9$ car le signal ne couvre jamais toute la \emph{range} de valeurs disponibles. Ce fait est moins important pour les algorithmes travaillant en \emph{floating point} mais il sera essentiel pour la mise à échelle des données en représentation \emph{fixed point} (section \ref{sec:fixed_point}).



    \paragraph{}
    Les MDCT de référence en \emph{floating point} prenant en entrée ces signaux sinusoïdaux à \SI{200}{\hertz} génèrent les spectres de fréquences présentés à la figure \ref{fig:validation_ref_mdct_float}. Le spectre généré par l'agorithme en \emph{float} (en pointillés) se superpose à celui généré en \emph{double} : les deux précision permettent donc bien de générer le même spectre.

    \paragraph{}
    La représentation graphique des spectres de fréquences met en évidence une composante fréquentielle principale aux alentours de \SI{200}{\hertz}. Le résultat obtenu est déjà satisfaisant puique la fréquence mise en évidence par le spectre est bien celle du signal d'entrée. Cependant, il est à noter que l'impléméntation d'une fonction de fenêtre appliquée au signal d'entrée permettrait d'améliorer encore le spectre. La section \ref{sec:ameliorations} consacrée aux améliorations possibles de ce travail expliquera ce qu'est cette fonction de fenêtre.
    ...
    \begin{figure}[H]
        \centering
        \includegraphics[width=.8\linewidth]{./images/validation_ref_mdct_float.pdf}
    ...
    \paragraph{}
    La figure \ref{fig:signal_i16} montre que le signal en \emph{fixed point} prend la même forme que celui en \emph{floating point} avec une période de \SI{5}{\milli\second} correspondant bien à la fréquence de \SI{200}{\hertz} attendue.

    \paragraph{}
    La valeur du signal varie cette fois entre $29491$ et $-29491$ qui corespondent aux valeurs limites du signal en \emph{floating point} mises à échelle pour une représentation \emph{fixed point} Q15.

    \begin{figure}[H]
        \centering
        \includegraphics[width=.8\linewidth]{./images/signal_i16.pdf}
        \caption{Signal sinusoïdal à \SI{200}{\hertz} codé en \emph{fixed point} (Q15)}
    ...
    \begin{figure}[H]
        \centering
        \includegraphics[width=.8\linewidth]{./images/validation_ref_mdct_int.pdf}
        \caption{}
...
% standard deviation: 1.36369e-07 ns
    ...
    \begin{figure}[H]
        ...
        \caption{Comparaison des spectres des fréquences produits par la MDCT Ne10 i32 plain C et par la MDCT de référence en \emph{integer}}
